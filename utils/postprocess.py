...
from constants.ldm import MODELS
from constants.datasets import COCO_CLASS_ID2NAME, LVIS_CLASS_ID2NAME
...
def process_bbox_mask(bboxes_person, masks_person, final_keepidx):
    return bboxes_person[final_keepidx], masks_person[final_keepidx]
        ...
        # else, check the bbox
            ...
            for idx, comparison_bbox in enumerate(bbox_list):
                ...
                # else, compare
                    ...
                    intersection_w = max([min(end_xs) - max(start_xs), 0.])
                    intersection_h = max([min(end_ys) - max(start_ys), 0.])
...
def process_segmentation(instances, minoverlap=0.8, exconf=0.98, verbose=False): # Use minoverlap=0.8, exconf=0.98!!!
    ...
    for class_id in list(set(instances.pred_classes.cpu().numpy().tolist())):
        ...
        masks_class = instances[is_class].pred_masks
...
def process_mocap_predictions(
    mocap_predictions, bboxes, image_size, masks=None
):
    ...
    inds = np.argsort(bboxes[:, 0])  # Sort from left to right
    person_parameters = {
        ...
        "indices": inds # --> indices for making the order from left bbox to right bbox
    }
