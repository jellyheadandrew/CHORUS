...
from constants.frankmocap import FRANKMOCAP_DIR

class BodyMocap(object):
    def __init__(self, regressor_checkpoint, smpl_dir, smplModelPath, device=torch.device('cuda'), use_smplx=False):
        ...
        if use_smplx:
            ...
            smplModelPath = smpl_dir + '/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl' if smpl_dir is not None else smplModelPath
        ...
        SMPL_MEAN_PARAMS = os.path.join(
            FRANKMOCAP_DIR,
            './extra_data/body_module/data_from_spin/smpl_mean_params.npz')
    ...
    def regress(self, img_original, body_bbox_list):
        ...
        for body_bbox in body_bbox_list:
            ...
            with torch.no_grad():
                ...
                # -> pred_rotmat: (B,24,3,3) where 23 are joint rotation matrices and 1 is global rotation matrix
                # -> pred_betas: (B,10) where 10 are number of betas
                # -> pred_camera: (B,3) where each are tx,ty,scale
                ...
                pred_joints_3d = pred_joints_3d[0].cpu().numpy() # (1,49,3) -> (49,3)
                ...
                pred_output['pred_joints_smpl'] = np.expand_dims(pred_joints_3d, axis=0) # shape: (1,49,3)
                ...
                # top-left coordinate in pixels, when origin is center of image (not top-left of image)
                # e.g. [-40 57]
                # given as np.ndarray
                ...
                # original bbox (of body)
                pred_output['body_bbox'] = body_bbox
                
                # SMPL faces
                pred_output['faces'] = self.smpl.faces # Faces. 
                ...
                # Used for predicting perspective camera when (ortho -> persp) transformation!
                # Check "test_optimize_camera_origin_fixed.ipynb" and "test_optimize_camera_origin"          
                smpl_output_zerorot = self.smpl(
                    betas=pred_betas, 
                    body_pose=pred_aa[:,3:],
                    global_orient=torch.zeros_like(pred_aa[:,:3]), 
                    pose2rot=True)
                pred_vertices_zerorot = smpl_output_zerorot.vertices
                pred_joints_3d_zerorot = smpl_output_zerorot.joints

                pred_vertices_zerorot = pred_vertices_zerorot[0].cpu().numpy()

                pred_vertices_bbox_zerorot = convert_smpl_to_bbox(pred_vertices_zerorot, camScale, camTrans)
                pred_vertices_img_zerorot = convert_bbox_to_oriIm(
                    pred_vertices_bbox_zerorot, boxScale_o2n, bboxTopLeft, img_original.shape[1], img_original.shape[0])

                # Convert joint to original image space (X,Y are aligned to image)
                pred_joints_3d_zerorot = pred_joints_3d_zerorot[0].cpu().numpy() # (1,49,3) -> (49,3)
                pred_joints_vis_zerorot = pred_joints_3d_zerorot[:,:3]  # (49,3)
                pred_joints_vis_bbox_zerorot = convert_smpl_to_bbox(pred_joints_vis_zerorot, camScale, camTrans) 
                pred_joints_vis_img_zerorot = convert_bbox_to_oriIm(
                    pred_joints_vis_bbox_zerorot, boxScale_o2n, bboxTopLeft, img_original.shape[1], img_original.shape[0]) 
                
                # Output
                pred_output['pred_vertices_smpl_zerorot'] = smpl_output_zerorot.vertices[0].cpu().numpy() # SMPL vertex in original smpl space
                pred_output['pred_vertices_img_zerorot'] = pred_vertices_img_zerorot # SMPL vertex in image space
                pred_output['pred_joints_img_zerorot'] = pred_joints_vis_img_zerorot # SMPL joints in image space
                pred_output['pred_joints_smpl_zerorot'] = np.expand_dims(pred_joints_3d_zerorot, axis=0) # shape: (1,49,3)
                
                smpl_output_zerorot_zerobeta = self.smpl(
                    betas=torch.zeros_like(pred_betas), 
                    body_pose=pred_aa[:,3:],
                    global_orient=torch.zeros_like(pred_aa[:,:3]), 
                    pose2rot=True)
                pred_vertices_zerorot_zerobeta = smpl_output_zerorot_zerobeta.vertices
                pred_joints_3d_zerorot_zerobeta = smpl_output_zerorot_zerobeta.joints

                pred_vertices_zerorot_zerobeta = pred_vertices_zerorot_zerobeta[0].cpu().numpy()

                pred_vertices_bbox_zerorot_zerobeta = convert_smpl_to_bbox(pred_vertices_zerorot_zerobeta, camScale, camTrans)
                pred_vertices_img_zerorot_zerobeta = convert_bbox_to_oriIm(
                    pred_vertices_bbox_zerorot_zerobeta, boxScale_o2n, bboxTopLeft, img_original.shape[1], img_original.shape[0])

                # Convert joint to original image space (X,Y are aligned to image)
                pred_joints_3d_zerorot_zerobeta = pred_joints_3d_zerorot_zerobeta[0].cpu().numpy() # (1,49,3) -> (49,3)
                pred_joints_vis_zerorot_zerobeta = pred_joints_3d_zerorot_zerobeta[:,:3]  # (49,3)
                pred_joints_vis_bbox_zerorot_zerobeta = convert_smpl_to_bbox(pred_joints_vis_zerorot_zerobeta, camScale, camTrans) 
                pred_joints_vis_img_zerorot_zerobeta = convert_bbox_to_oriIm(
                    pred_joints_vis_bbox_zerorot_zerobeta, boxScale_o2n, bboxTopLeft, img_original.shape[1], img_original.shape[0]) 
                
                # Output
                pred_output['pred_vertices_smpl_zerorot_zerobeta'] = smpl_output_zerorot_zerobeta.vertices[0].cpu().numpy() # SMPL vertex in original smpl space
                pred_output['pred_vertices_img_zerorot_zerobeta'] = pred_vertices_img_zerorot_zerobeta # SMPL vertex in image space
                pred_output['pred_joints_img_zerorot_zerobeta'] = pred_joints_vis_img_zerorot_zerobeta # SMPL joints in image space
                pred_output['pred_joints_smpl_zerorot_zerobeta'] = np.expand_dims(pred_joints_3d_zerorot_zerobeta, axis=0) # shape: (1,49,3)
