...
def run_body_mocap(args, body_bbox_detector, body_mocap, visualizer):
    ...
    cur_frame = args.start_frame # --> Default: 0 (Check demo_options.py)
    # video_frame = 0
    # timer = Timer()
    while True:
        # timer.tic()
        ...
        body_bbox_list = [ body_bbox_list[i] for i in idx_big2small ] # If bbox is big, it comes in front
        if args.single_person and len(body_bbox_list)>0: # args.single_person: Just reconstruct one person (with biggest bbox)
